...
const ApprovedLeaveFirstForApprover = ({ name = 'Jomel', approverRole = "Random", lastApprover = { name: 'Sample1', role: 'Role'} }: ApprovedLeaveFirstForApproverProps) => {
  return (
      <Html>
         <Head />
                   ...
                   <Text>Hi {approverRole},</Text>
                    <Text className='mb-10'>
                     Employee
                      ...
                      has requested a leave and is waiting on your approval for completion
                    </Text>
                    <Text>
                      This leave was recently approved by
                      <span className='mx-1 font-bold p-[2px]'>{lastApprover.name}</span>({lastApprover.role})
                    </Text>
                    
                    {/** @ts-ignore */}
                    Please login to <Link clicktracking="off" href={process.env.APP_LINK}>{process.env.APP_LINK_2}</Link> to approve
               </Container>
            </Body>
         </Tailwind>
      </Html>
