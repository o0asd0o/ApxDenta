...
>(
  ...
  const rows = await qb.limit(opts.limit + 1).execute();

  const hasNextPage = reversed ? undefined : rows.length > opts.limit;
  const hasPrevPage = !reversed ? undefined : rows.length > opts.limit;

  // If we fetched an extra row to determine if we have a next page, that
  // shouldn't be in the returned results
  if (rows.length > opts.limit) rows.pop();
