...
const PatientsCardLayout: React.FC<LayoutProps> = ({
  ...
  const {
    data: patientsList = { pages: [], pageParams: [] },
    isLoading,
    fetchNextPage,
    hasNextPage,
    refetch,
  } = useInfiniteQuery({
    queryKey: ['patientsList', { isActive, filters, pagination, sorting }],
    queryFn: ({ pageParam = 1 }) => {
      return trpc.patients.getAllPatients.query({
        isActive,
  ...
  const flatten = patientsList.pages.flatMap((page) => page.data);
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        {isLoading && (
          <div className="h-[400px] justify-center items-center w-full col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4">
